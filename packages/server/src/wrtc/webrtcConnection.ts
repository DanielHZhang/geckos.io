...
import type {RTCIceCandidate} from 'werift/lib/rtc/transport/ice';
...
export default class WebRTCConnection extends Connection {
  ...
  constructor(
    ...
    this.peerConnection = new RTCPeerConnection({stunServer: ['stun.l.google.com', 19302]});
    this.peerConnection.iceConnectionStateChange.subscribe((data) => {
  ...
  async doOffer() {
    try {
      ...
      this.waitUntilIceGatheringStateComplete(this.peerConnection, this.options);
    } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        if (totalIceCandidates > 0) {
          ...
          reject(
            new Error(
              'Timed out waiting for host candidates State: ' + peerConnection.iceGatheringState
            )
          );
        }
      ...
      peerConnection.onIceCandidate.once((candidate) => {
        // const { candidate } = ev;
