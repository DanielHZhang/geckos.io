...
import {CorsOptions} from '@rtcweb/common/lib/types';
...
const HttpServer = (
  ...
  server.on('request', async (req: http.IncomingMessage, res: http.ServerResponse) => {
    ...
    if (pathname && rootRegEx.test(pathname)) {
      ...
      const path2 = new RegExp(
        `${prefix}\/${version}\/connections\/[0-9a-zA-Z]+\/remote-description`
      ).test(pathname);
      const path3 = new RegExp(
        `${prefix}\/${version}\/connections\/[0-9a-zA-Z]+\/additional-candidates`
      ).test(pathname);
      const closePath = new RegExp(`${prefix}\/${version}\/connections\/[0-9a-zA-Z]+\/close`).test(
        pathname
      );
